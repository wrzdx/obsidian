...
- **Дискретные значения как `display` или `content-visibility` переключаются после половины анимации**
	...
	Когда мы изменяем `display`  во время перехода, мы должны установить `@starting-style` что предоставить начальные параметры, это необходимо чтобы избежать неопределенного поведения
