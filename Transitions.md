...
- **Дискретные значения как `{css}display` или `{css}content-visibility` переключаются после половины анимации**
	...
	Примерно такая же ситуация когда используем js для изменения макета, браузер не знает, и чтобы правильно отрендерить переход надо `{js}setTimeout()` использовать.

- **Используем `{css}transitionend`,  `{css}transitionrun`,  `{css}transitionstart` события чтобы узнать то переход закончен, началось до задержки, началось после задержки соответственно.**
...
- **Используем `{css}isolation: isolate` для того чтобы сделать изолированную компоненту которая не будет конфликтовать с внешним миров**
...
- **Можно заранее сказать браузеру создать объект на другом слое чтобы все другие слои пропустили некоторые стадии отрисовки страницы. Для этого используют `{css}will-change`. Однако надо измерять performance. Кроме того, объект начинает сразу рендерится GPU, а не передаваться от CPU к GPU и обратно, и благодаря этому нет прыжков и искажений, но потребление памяти чуть-чуть возрастает**

- **Использование GPU (`{css}transform`) также нам дает доступ к подпикселям, что не доступно CPU операциям e.g. `{css}margin`, из-за чего переход более плавный**
